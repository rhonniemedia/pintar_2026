    ...
    <x-ui.modal show="open" maxWidth="md">
        ...
        <div class="flex items-start sm:items-center justify-between gap-3 px-4 sm:px-6 py-4 sm:py-5 border-b border-border bg-slate-50/50 shrink-0">
            <div class="flex items-center gap-3 sm:gap-4 min-w-0">
                <div class="size-11 sm:size-12 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center shrink-0 shadow-sm border border-blue-200">
                    <i data-lucide="book-open" class="size-5 sm:size-6"></i>
                </div>

                <div class="min-w-0">
                    <h3 class="font-bold text-foreground text-base sm:text-lg leading-tight truncate">Cetak Rekap Jurusan</h3>
        ...
        <form action="{{ route('admin.students.reports.concentration') }}" method="GET" target="_blank" @submit="setTimeout(() => closeModal(), 200)">
            {{-- Body Modal --}}
            <div class="p-4 sm:p-6 overflow-y-auto max-h-[60vh] bg-slate-50/30 flex-1 custom-scrollbar space-y-4">

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium mb-1.5 text-foreground">Bulan Awal</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1.5 text-foreground">Bulan Akhir</label>
            ...
            <div class="px-4 sm:px-6 py-3.5 sm:py-4 border-t border-border bg-slate-50/50 flex items-center justify-end gap-3 shrink-0">
                ...
                <button type="submit"
                    class="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-70 disabled:cursor-not-allowed text-white rounded-xl font-semibold text-sm transition-all shadow-sm shadow-blue-600/30 cursor-pointer">
