<div x-data="{
    ...
    <div class="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <div class="bg-error/10 text-error px-6 py-3 text-sm font-medium border-b border-error/20 flex gap-2 items-center shrink-0">
            <i data-lucide="alert-triangle" class="size-4"></i>
            Semester berikutnya belum diatur. Harap buat semester baru di Data Master sebelum memproses kenaikan.
        </div>
        @endif

        {{-- Form --}}
        <form action="{{ route('admin.students.group.promote', $classGroup->id) }}" method="POST"
            @submit="saving = true"
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-border/50 hover:bg-slate-50 transition-colors"
                                ...
                                <td class="py-3 text-sm flex gap-2 items-center">
                                    ...
                                    {{-- Pastikan ini disesuaikan dengan cara Anda memanggil NISN seperti di _graduation-modal --}}
            ...
            <div class="px-6 py-4 border-t border-border bg-gray-50/50 flex flex-col lg:flex-row items-center justify-between shrink-0 gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <select name="target_class_group_id" x-show="decision === 'naik'" x-cloak class="bg-white border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary transition-all min-w-[220px]" :required="decision === 'naik'" :disabled="decision !== 'naik'">
                    ...
                    <select name="target_class_group_id" x-show="decision === 'tinggal'" x-cloak class="bg-white border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary transition-all min-w-[220px]" :required="decision === 'tinggal'" :disabled="decision !== 'tinggal'">
                    ...
                    <input type="date" name="entry_date" value="{{ date('Y-m-d') }}" required class="bg-white border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-primary transition-all">
                </div>
