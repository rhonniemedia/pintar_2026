<div id="modal-container"
    ...
    <x-ui.modal show="open" maxWidth="2xl">
        ...
        <div id="create-modal-content" class="flex flex-col flex-1 min-h-0 h-full w-full"
            ...
            $concOptions = [];
            if(isset($concentrations)) {
            foreach ($concentrations as $concentration) {
            $concOptions[] = ['value' => $concentration->id, 'label' => $concentration->name];
            }
            }
            ...
            ['value' => 'transfer', 'label' => 'Siswa Pindahan'],
            ];
            @endphp
            ...
            <div class="block p-4 sm:p-6 overflow-y-auto bg-slate-50/30 flex-1 [scrollbar-gutter:stable]">
                ...
                <form id="create-student-form"
                    ...
                    hx-post="{{ route('admin.students.data.store') }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-white border border-border rounded-2xl">
                        ...
                        <div>
                            ...
                            <x-ui.select
                                ...
                                value="{{ old('entry_grade_level') }}"
                                placeholder="— Pilih Kelas —" />
                            @error('entry_grade_level') <span class="text-error text-[10px] mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="{{ $labelClass }}">Program Keahlian <span class="text-error">*</span></label>
                            <x-ui.select
                                name="concentration_id"
                                :options="$concOptions"
                                value="{{ old('concentration_id') }}"
                        ...
                        <div>
                            ...
                            <x-ui.select
                                ...
                                value="{{ old('registration_type', 'new') }}"
